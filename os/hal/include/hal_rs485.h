...
#define RS485_MAX_ACK     2  // 0b0 - 0b1
...
/*
#define RS485_FLAG_CMD       0b10
#define RS485_FLAG_DTA       0b00
#define RS485_FLAG_MULTI_DTA 0b01
*/
/**
 * @name    Serial status flags
 * @{
 */
#define SD_PARITY_ERROR              CHN_PARITY_ERROR    /**< @brief Parity.     */
#define SD_FRAMING_ERROR             CHN_FRAMING_ERROR   /**< @brief Framing.    */
#define SD_OVERRUN_ERROR             CHN_OVERRUN_ERROR   /**< @brief Overflow.   */
#define SD_NOISE_ERROR               CHN_NOISE_ERROR     /**< @brief Line noise. */
#define SD_BREAK_DETECTED            CHN_BREAK_DETECTED  /**< @brief LIN Break.  */
 ...
 */
typedef struct {
  uint8_t address; // less then RS485_MAX_ADDRESS
  uint8_t length;  // less than RS485_MSG_SIZE
  uint8_t ctrl;    // less than RS485_MAX_CTRL
  uint8_t ack;     // less than RS485_MAX_ACK
  uint8_t data[RS485_MSG_SIZE];
} RS485Msg_t;

typedef struct {
  uint8_t address; // less than RS485_MAX_ADDRESS
  uint8_t length;  // less than RS485_MSG_SIZE
  uint8_t ctrl;    // less than RS485_MAX_CTRL
  uint8_t ack;     // less than RS485_MAX_ACK
} RS485Cmd_t;
 ...
 *
 * @brief   Half duplex RS485 driver class.
