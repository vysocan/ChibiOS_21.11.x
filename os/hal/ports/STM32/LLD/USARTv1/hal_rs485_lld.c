 ...
 */
/**
 * @file    USARTv1/hal_rs485_lld.c
 * @brief   STM32 RS485 low level serial driver code.
 *
 * @addtogroup RS485
 * @{
 */
 ...
 */
static void rs485_init(RS485Driver *rs485p, const RS485Config *config) {
  ...
  u->CR1 = ( USART_CR1_UE | USART_CR1_PEIE | USART_CR1_RWU |
             USART_CR1_RXNEIE | USART_CR1_TE | USART_CR1_M |
             USART_CR1_RE); //& ~USART_CR1_TCIE;

  // Cover GW and nodes, nodes need to respond to own address and multicast
  if (config->address == 0) {
    u->CR2 = config->address & USART_CR2_ADD; // Address at ADD[0:3] masked by 0b1111
    u->CR1 |=  USART_CR1_WAKE;
  } else {
    u->CR2 = 0;
  }
}
...
static void serve_interrupt(RS485Driver *rs485p) {
  ...
  while (sr & (USART_SR_RXNE | USART_SR_ORE | USART_SR_NE | USART_SR_FE |
    ...
    if (sr & USART_SR_RXNE) {
      ...
      sr = u->SR;
      // Gateway is uses HW address, and it does not need to respond to broadcast
      if (rs485p->config->address == 0) {
        rs485IncomingDataI(rs485p, (uint8_t)(b & 0xFF));
      } else {
        // Check first byte and
        // Check if b is our address or broadcast (0x1111)=(USART_CR2_ADD)
        uint8_t test = (uint8_t) b & USART_CR2_ADD;
        if ((rs485p->trcState == TRC_READY) &
            ((rs485p->config->address != test) & (USART_CR2_ADD != test))) {
            // Not us, go to invalidate
...
msg_t rs485_lld_SendMsgWithACK(RS485Driver *rs485p, RS485Msg_t *datap, uint8_t repeat, uint8_t ackFlag){
  ...
  osalDbgCheck(ackFlag < RS485_MAX_ACK);            // ACK is allowed
  ...
  while (count != repeat) {
    ...
    if (rs485p->trcState == TRC_READY) {
      ...
      // Enable TX
      rs485p->usart->CR1|= USART_CR1_TXEIE;
